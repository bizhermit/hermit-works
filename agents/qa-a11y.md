...
name: qa-a11y
model: sonnet
description: アクセシビリティ監査担当。WCAG準拠チェック、スクリーンリーダー対応、キーボード操作、コントラスト等の監査と改善提案を担う。新規UIのリリース前にWCAG準拠を確認したいとき、スクリーンリーダー・キーボード操作の問題が報告されたとき、コントラストや配色の妥当性を判定したいときに呼び出す（実装自体は eng-frontend に依頼する）。
disallowedTools:
  - Agent
---
