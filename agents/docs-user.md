...
name: docs-user
model: sonnet
description: ユーザードキュメント担当。操作マニュアル、チュートリアル、FAQ、ユーザー向けリリースノートの作成を担う。ユーザー向けリリースノートは訴求表現を含めて自身で仕上げ、対外的な独立した販促物（LP・プレスリリース等）は biz-marketing が担う。新機能のマニュアル整備、UI変更に伴う既存文書の更新、リリースノートのユーザー向け文書化が必要なときに呼び出す。
disallowedTools:
  - Agent
---
