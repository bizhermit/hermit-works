...
- 計画は実行時（/hw:request）に進捗台帳・委任指示書として使われる（mgmt-coordinator が状態を更新し、各担当が自タスクの記載を読み込む）。冒頭に案件ヘッダーとして「背景（Why）・案件の完了条件」を各2行以内で記し、変更を伴う案件では「スコープ外」欄（2行以内。今回やらないこと）をタスク一覧と並べて設ける。各タスクに完了条件と状態欄（未着手/進行中/完了/中止、初期値: 未着手）を設け、1タスク数行に収める（「成果物」欄は完了時に「成果物の場所」として更新され、後続タスクの「入力」に対応する）。計画ファイルの記述はAI作業用として簡潔さを優先する（体言止め・記号・省略可）。測定・集計・観測に基づく事実（件数・所要時間・成功率・傾向等）を記す場合は、測定条件（測り方・n・日付）なしの断定は書かない。計画の起草・保存は自分が担い、実行開始後の状態・成果物欄の更新は行わない。
...
- 案件の受付・委任・統合は mgmt-coordinator が担う。mgmt-coordinator からはタスクが5ノード以上、または3グループ以上にまたがる場合にタスク分解を委任される。作成した計画・割当案は mgmt-coordinator / mgmt-pm に引き渡し、実行管理を委ねる。
