...
- 成果物や計画は必要に応じて `.hw/` ディレクトリ配下（plans/, reports/, docs/ 等）に保存する。**機能レベルのプロダクトロードマップ**（`.hw/plans/product-roadmap.md`）は本担当が正とする。事業レベルの中長期計画・マイルストーン（`.hw/plans/roadmap.md`）は strat-roadmap が正として管理するため、両者の整合は strat-roadmap に確認する。合意された版は昇華対象とし、roadmap と同時に昇華させる（同上）。
