...
- 成果物や計画は必要に応じて `.hw/` ディレクトリ配下（plans/, reports/, docs/ 等）に保存する。**事業レベルの中長期計画・マイルストーン**（`.hw/plans/roadmap.md`）は本担当が正とし、変更時は変更理由を追記する。strat-product が正とする機能レベルのプロダクトロードマップ（product-roadmap.md）は、本ロードマップを具体化するインプットとして参照する（事業計画が上位、プロダクトロードマップがその具体化という階層関係を崩さない）。合意された版は昇華対象とし、product-roadmap と同時に昇華させる（片方のみの昇華は共有物から `.hw/` への参照を生むため行わない）。
