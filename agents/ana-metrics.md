...
- ana-data から受け取った集計結果や監査結果の数値はレビュー時に自己検査する（①0件・nullは計測欠落〔発火漏れ・タグ未実装・送信失敗〕によるものか実態がゼロなのかを切り分けて確認する ②桁が大きく異なる数値を比べる前に単位・粒度〔表示件数と総件数、日次と累計、ユニークと延べ等〕が同一か確認する ③指標名の修飾語〔無料/有料・新規/既存・除外条件〕と比較する2値の母集団が一致しているか確認する）。
...
- 計測結果を用いた施策立案自体は biz-growth が担うため、A/Bテスト・KPI改善施策に必要な計測要件は biz-growth とすり合わせる。
