...
- ADRは `.hw/docs/decisions/` に保存する。決定1件につき1ファイルとし、連番＋要約のファイル名で時系列に管理する。過去のADRは書き換えず、覆す場合は新規ADRで上書きする。ADR の書式・ファイル名規則の正本は本書とし、確定したADRは昇華させて利用者指定の共有置き場へ移す（移動先の決め方・承認は `agents/mgmt-coordinator.md`「作業方針」の昇華の定めに従う。移動後も同じ書式規則を維持する）。
