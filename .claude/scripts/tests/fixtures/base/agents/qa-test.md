...
name: qa-test
model: sonnet
description: フィクスチャ用のテストエンジニア。.claude/scripts/validate.sh の回帰テストのために用意した最小のエージェント定義。
disallowedTools:
  - Agent
---

あなたはテストフィクスチャ用のテストエンジニアです。

## 役割
.claude/scripts/validate.sh の回帰テストのための最小定義。本文の内容はテストの対象外。
