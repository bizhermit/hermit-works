...
hermit-works 組織の構成を案内します。以下の組織図を、各エージェントの一行説明付きの表として既定の言語（利用者側に言語規約があればそれに従い、なければ日本語）で表示してください（本文の一覧は `agents/*.md` の frontmatter から生成済みのため、ファイルの追加読み込みは不要）。
