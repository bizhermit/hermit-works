...
10. **タグ付与**: 利用者からマージ完了の報告を得たら、タグ付与の前にマージ完了を機械的に検証する
    （申告のみに依拠しない）。
    - `gh pr view <PR番号> --json state,mergedAt,mergeCommit` を実行し（`<PR番号>` は手順8で
      控えたもの）、`state` が `MERGED` であることを確認し、あわせて `mergeCommit.oid`
      （マージコミットの SHA）を取得する。
    - `git fetch origin` 後、`git log -1 --format=%P <mergeCommit.oid>`（または
      ...
      ない場合は squash または rebase マージが行われた疑いがあるため、**タグ付与を中断**し、
      想定と異なる履歴である旨を利用者に報告し、指示を仰ぐ（以降の手順は進めない）。
    - 上記2点を確認できたら、タグ `v<version>` を付与してよいか個別に確認を得てから、手順6で
...
11. **タグ push**: タグ付与後、リモートへのタグ push を実行してよいか改めて個別に確認を得てから
    push する。

12. **完了報告**: 実施内容（発火判定の結果・確定した version と種別・コミット/push/PR/マージ確認/
    タグ/タグpush の実施状況）を簡潔に報告する。手順1または3で終了した場合はその旨と理由のみを
    報告する。
