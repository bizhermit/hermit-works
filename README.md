...
- 生成担当と評価担当を分離した品質ゲート（`qa-review`・`sec-audit` 等によるレビュー→差し戻し→再評価。差し戻し上限は既定3回で利用者側が調整可、sec-audit / sec-privacy の起動条件は下限固定で緩和不可）
...
- 実行環境の上限（同時実行数・セッション累計等）到達時の委任・品質ゲートの維持（縮退し、統括はその旨を報告します）
- 統括をサブエージェントとして起動した場合の利用者確認（`AskUserQuestion` 等）の実行（確認を要する手続きは呼び出し元への完了報告に回ります）
...
| `/hw:standup [対象]` | 進捗の状況報告（スタンドアップ） |
| `/hw:review [対象]` | 品質＋セキュリティ＋テスト観点の総合レビュー |
| `/hw:report [重点観点]` | 定期レポート（進捗・品質・セキュリティ・技術負債）。定期実行向け |
...
5. **評価ループ** — 生成ノードと評価ノード（`qa-review` `sec-audit` 等）を分離し、指摘→改善→再評価のループを回す（差し戻し上限は既定3回で利用者側が調整可）
...
| `hw:permission-rules` | Claude Code の Bash パターンマッチング・ファイルパス glob（Edit()/Read() のパスルール、Write() 等にも及ぶ hooks の `if:` 条件）・permissions 運用ガイダンス。deny ルール提案時の詳細規則・確認手順・多層防御考慮を提供する。sec-audit・sec-appsec・infra-devops が permissions や hooks の `if:` 条件を提案・変更・レビューする際に読み込む |
...
2. `/hw:optimize-assets` — 資材**構成**の最適化。本プラグインとの適合性（役割の重複＝独自エージェント/
   コマンドと hw 組織、および Claude Code 本体のバンドルスキル・他プラグイン提供スキルとの被り、指示の矛盾＝CLAUDE.md と hw
   ワークフローの衝突、指示の分散＝他ツール向け資材にだけある規約、動作の阻害＝permissions / hooks）
   に加え、利用者資材**同士**の乱立（重複・断片化・死蔵）も検出し、承認のうえで統廃合・移行します
...
├── docs/             # その他の組織成果物（ADR・ロードマップ等。確定後は昇華してチーム共有の資材置き場へ移動）
...
`docs/` に保存された成果物（ADR・ロードマップ等）は、`procedures/` の「昇格」とは別の仕組みとして、
内容が確定すると「昇華」し、`.claude/` または利用者が指定した資材置き場（`docs/` 等）へ移動します。
昇華は利用者の承認を得てから行われ、移動後は原本を `.hw/` 側に残しません（`decisions.md` のみ例外で、
該当行に移動先を追記したうえでそのまま残ります）。
