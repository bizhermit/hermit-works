...
システム開発（Web / デスクトップ / モバイル）向けの Claude Code プラグイン。
モノレポ内に複数のプロジェクト（≒マイクロサービス）が同居するリポジトリを対象に、**10グループ・51名の専門家エージェント**が人間の指示や定期的な指示を受けて作業を分担します。

> 本ドキュメントは**プラグインを利用する方**向けです。リポジトリの保守・開発に参加する方は
> [DEVELOPMENT.md](DEVELOPMENT.md)（保守ガイド）と [CONTRIBUTING.md](CONTRIBUTING.md)（追加・変更手順）を参照してください。
...
導入検討のため、本プラグインが担う範囲・担わない範囲と、導入して得られることを記します。
...
| `/hw:release <対象>` | リリース準備・判定チェック・リリースノート案 |
...
1. `/hw:init` — 作業ディレクトリの作成と資材の索引化（モノレポ構成マップ・利用者資材マップの作成）
