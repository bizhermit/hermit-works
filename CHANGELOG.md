...
### Added
- `hw:permission-rules` スキルにファイルパス glob パターンの規則（`Edit()`/`Read()` のパスルール、
  `Write()` 等にも及ぶ hooks の `if:` 条件）と sandbox 設定層の規則を追加（OS 境界・全サブプロセス
  適用・フェイルオープン特性・加算のみの位置づけ、`strictAllowlist`/`filesystem.disabled` の設定
  ソース制限と誤設定事故への注意）。規則ごとに出典区分（実機検証済み／公式ドキュメント確認のみ）を
  明示し、多層防御の案内を追加（DESIGN 2.3 の層③を sandbox を内訳として再定義）
- `/hw:optimize-assets` の役割重複検出に Claude Code 公式バンドルスキルとの比較軸を追加（利用者独自
  資材が公式スキルと重複していないかの点検）
- `scripts/validate.sh` に文書内パス参照の実在検証（セクション12）を追加し、文書間リンクの静かな
  腐敗を機械検知（テストハーネスへのケース追加込み）

### Changed
- 品質ゲート受入時に、レビュー担当が申告したレビュー対象一覧と実際の変更ファイル一覧の突合を必須化
  （未レビューのファイルを残したままの合格を防止。`mgmt-coordinator`・`qa-review`・`sec-audit`・
  `sec-privacy`・`/hw:request`）
- 測定・集計に基づく事実記録に、測定条件（測り方・n・日付）の記載を必須化（`mgmt-coordinator`・
  `mgmt-planner`・`/hw:request`）
- `ai-prompt` の評価基準に複数回試行・ブレ幅測定の規律を追加（改善前後の比較は同条件・同回数の複数回
  試行で行い単一実行同士で優劣を断じない、平均・最低点・ブレ幅での判定、FN/FP 区別と業務上の重大度に
  よる重み付け、試行回数・日付・対象モデルの報告付記、評価基盤が無い場合の整備先行提案）。`ai-lead`
  にも単発実行の結果のみで採否・品質を裁定しない旨を追記
- `repo-map`・`conventions` スキルを `context: fork` 実行へ変更し、呼び出し元のコンテキスト汚染を
  防止（差分の非要約列挙・フォーク失敗時フォールバック・転記抑制・秘密情報非転記を規定。非リーダーから
  の fork スキル呼び出しを検出する検査を `scripts/validate.sh` に追加）
- サブエージェント実行上限到達時の縮退動作を統括の作業方針に明記（上限到達時の分担計画フォールバック、
  評価ノード起動不能時は品質ゲート未実施であることを明示して利用者判断へ、ファンアウトの分割順次起動）。
  README「できないこと」にも1行追記
- URL 区切り規約を `commands/` 全12ファイルへ展開し、`scripts/validate.sh` の逐語検証で機械担保
  （利用者向け出力で URL を装飾記号・全角括弧で挟みリンク不能にする事故の再発防止。正本と docs 系
  エージェントへ装飾記号の具体例も追記）
- `/hw:standup` の課題表を状態で「完了以外」（未着手・進行中）と「完了」（完了・見送り）の2節に分割
  して表示（No. は節をまたぐ表示順の通し番号を継続、各節内の並び順は現行維持、該当0件の節は小見出しを
  残して「該当なし」と記載）
